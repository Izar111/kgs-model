...
def load_tokenizer(module_name=None):
    if type(module_name) is str:
        return KoBertTokenizer.from_pretrained(module_name)
...
def get_device(no_cuda=None):
    device_str = "cuda"
    if no_cuda is not None and not torch.cuda.is_available() or no_cuda:
        device_str = "cpu"
...
def predict(model=None, model_dir=None, lines=None, input_file_dir=None, module_name="monologg/kobert", no_cuda=0, batch_size=64, model_type="bert", max_seq_len=100, links=None):
    # set default values
    if lines is None:
    ...
    for words, preds, link in zip(lines, preds_list, links):
        ...
        for e1, e2 in pair:
            ...
            present.append((e1_object, e2_object, links))
